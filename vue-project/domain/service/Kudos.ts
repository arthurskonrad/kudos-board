...
export default class KudosService {
  ...
  async index({ userId, panelSlug }: { userId: string; panelSlug: string }) {
    try {
      const response = await fetch(`http://mock-api:8080/api/panels/${panelSlug}/kudos`, {
        headers: {
  ...
  async findBySlug({ kudosSlug, userId }: { kudosSlug: string; userId: string }) {
    try {
      const response = await fetch(`http://mock-api:8080/api/kudos/${kudosSlug}`, {
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
          'user-id': userId
